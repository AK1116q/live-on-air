# v0.1.2 Validation Notes

Date: 2026-09-12. Environment: Windows, Node.js 24.11.0. Browser checks use Playwright with the local Edge installation.

- Node tests cover room parsing, settings validation, provider response mapping, notification text, storage updates, transition-only notifications, duplicate room prevention, and popup CSS sizing regressions.
- Edge loads the actual Manifest V3 extension with a temporary browser profile.
- The popup UI is checked for adding Bilibili and Douyu rooms, settings updates, manual polling, stable popup width, hidden horizontal overflow, and responsive layout.
